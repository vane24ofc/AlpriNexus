...
interface UserProfileData {
  id?: string; // Añadido opcionalmente para consistencia
  name: string;
  email: string;
  avatarUrl?: string | null; // Añadido para consistencia
}
...
export default function DashboardLayout({
  ...
  const fetchAndSetSession = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/me'); // El middleware se encarga de la cookie
      if (response.ok) {
        ...
        // Si /api/me falla (ej. 401, 403), es que la sesión no es válida o no existe
        localStorage.removeItem(SESSION_ROLE_STORAGE_KEY);
        localStorage.removeItem(USER_PROFILE_STORAGE_KEY);
        router.push('/login?sessionError=true'); // Redirigir al login
      }
    } catch (error) {
      console.error("Error fetching session from /api/me:", error);
      localStorage.removeItem(SESSION_ROLE_STORAGE_KEY);
      localStorage.removeItem(USER_PROFILE_STORAGE_KEY);
      router.push('/login?fetchError=true'); // Redirigir al login
    } finally {
  ...
  useEffect(() => {
    ...
    // Intentar cargar desde localStorage primero para una carga de UI más rápida
    ...
    if (roleFromStorage && storedProfileString) {
      try {
        ...
        // Sigue llamando a fetchAndSetSession para verificar con el backend y sincronizar
        fetchAndSetSession();
      } catch (e) {
        console.error("Error parsing stored data, fetching from API:", e);
        fetchAndSetSession(); // Si localStorage está corrupto, confía en la API
      }
    } else {
      // Si no hay datos en localStorage, llama a /api/me directamente.
      // Si la sesión no es válida, /api/me (o el middleware) redirigirá a login.
      fetchAndSetSession();
    }
  ...
  if (!hasMountedLayout || isLoadingRole || !currentSessionRole) {
    // Si currentSessionRole es null después de intentar cargar (y isLoadingRole es false),
    // fetchAndSetSession ya debería haber redirigido a login.
    // Este loader es para el estado de carga inicial.
